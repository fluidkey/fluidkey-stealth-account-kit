...
export function generateEphemeralPrivateKey({
  ...
  assert(
    ...
    'Either coinType or chainId must be defined.'
  );
  ...
  const coinTypeString = (coinType as number).toString(16).padStart(8, '0');
  ...
  return { ephemeralPrivateKey: toHex(childPrivateKey.privateKey as Uint8Array) };
}
